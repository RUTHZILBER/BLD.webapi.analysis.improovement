...
namespace BLL
{
    public class DB
    {
        ...
        /// <returns></returns>
        public List<string> getTags(string song, out string name, out int indexBest, out int indexSecond, out int indexThird)
        {

            List<string> wordsSong = att.startUp(song, out indexBest, out indexSecond, out indexThird);
            name = wordsSong[0];
            return wordsSong;
        }
        /// <summary>
        ...
        public RequestResult insertSong(DtoSong dtoSong, int userId = 0)
        {
            ...
            try
            {
                ...
                int indexBest = 0, indexSecond = 0, indexThird = 0;
                List<string> tagList = getTags(song.songContect, out songName, out indexBest, out indexSecond, out indexThird);//קבלת רשימת התגיות לפי הנתוח הנ"ל
                                                                                                                               //tagList = tagList.Distinct().ToList();//הסרת תגיות חוזרות על עצמן
                                                                                                                               //השיר הוא התגית עם האינדקס הכי גבוה, עם הניקוד הרב ביותר: 70
                songName = tagList[indexBest];
                ...
                foreach (var item in tagList)//הכנת התגיות למסד
                {
                    index++;
                    if(index==indexBest)
                        ent.Tags.Add(new DAL.Tag { points = 90, tagName = item, songId = song.songId });
                    if(index==indexSecond)
                        ent.Tags.Add(new DAL.Tag { points = 70, tagName = item, songId = song.songId });
                    if(index==indexThird)
                        ent.Tags.Add(new DAL.Tag { points = 50, tagName = item, songId = song.songId });
                    if(index!=indexBest&&index!=indexSecond&&index!=indexThird)
